...
export default {
  ...
  created() {
    this.getGameGradeData();
  },
  methods: {
    // 获取所有字典表下拉list
    getGameGradeData() {
      getGameGradeData().then((res) => {
        const result = res.data;
        if (result.code === 0) {
          this.gamesGradeList = result.data;
        } else {
    ...
    gameRuleFileRemove(file) {
      this.form.gameRuleName.forEach((item, index) => {
        if (item === file.name) {
          this.form.gameRuleName.splice(index, 1);
          this.form.gameRulePath.splice(index, 1);
        }
      });
    },
